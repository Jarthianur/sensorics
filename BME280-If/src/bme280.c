...
/*void printbin(uint32_t i)
 {
 for (uint32_t s = 0x80000000; s > 0; s >>= 1)
 {
 printf("%c", ((i & s) == s) ? '1' : '0');
 }
 printf("\n");
...
uint8_t bme280_set_powermode(uint8_t mode)
{
    ...
    if (mode <= BME280_NORMAL_MODE)
    {
        ...
        v_prev_pow_mode_uint8_t = bme280_get_powermode();
...
uint8_t bme280_get_humid_oversample()
{
    ...
    p_bme280->ovrsmpl_humid = get_bit_slice(val, 0x07, 0);
...
uint8_t bme280_compensate_temp()
{
    ...
    var2 =
        ((((double)adc_T) / 131072.0 - ((double)calib->dig_T1) / 8192.0) * (((double)adc_T) / 131072.0 - ((double)calib->dig_T1) / 8192.0)) * ((double)calib->dig_T3);
...
uint8_t bme280_compensate_humid()
{
    ...
    var_H = (adc_H - (((double)calib->dig_H4) * 64.0 + ((double)calib->dig_H5) / 16384.0 * var_H)) * (((double)calib->dig_H2) / 65536.0 * (1.0 + ((double)calib->dig_H6) / 67108864.0 * var_H * (1.0 + ((double)calib->dig_H3) / 67108864.0 * var_H)));
...
uint8_t unpack_press()
{
    NULL_CHECK
    bme280_uncomp_meas *meas = p_bme280->p_uncomp_meas;
    meas->pressure = (int32_t)((((uint32_t)meas->pmsb) << 12) | (((uint32_t)meas->plsb) << 4) | ((uint32_t)meas->pxsb >> 4));
    return U8_SUCCESS;
}
...
uint8_t unpack_temp()
{
    NULL_CHECK
    bme280_uncomp_meas *meas = p_bme280->p_uncomp_meas;
    meas->temperature = (int32_t)((((uint32_t)meas->tmsb) << 12) | (((uint32_t)meas->tlsb) << 4) | ((uint32_t)meas->txsb >> 4));
    return U8_SUCCESS;
}
